<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/css/bootstrap.min.css" integrity="sha384-ggOyR0iXCbMQv3Xipma34MD+dH/1fQ784/j6cY/iJTQUOhcWr7x9JvoRxT2MZw1T" crossorigin="anonymous">
    <link rel="stylesheet" href="css/main.css">
    <title>Ten</title>
</head>
<body>
<div class="header">
  <center><h1>This is the Main Header Area..!</h1></center>
</div>
<hr>
<div class="menu">
  <ul>
      <h2>Menu Bar</h2>
    <li> <a href="cregister.php">Registeration</a> </li>
    <li><a href="clientmaster.php">Client Master</a> </li>
    <li><a href="complaindetails.php">complain Details</a> </li>
    <li><a href="clientreport.php">Client Report</a> </li>
  </ul>
</div>

<div class="main">
 <center> <h1>Main content part Area</h1></center>

 <!-- login page div -->

  <div class="row" id="loginpage">
   
  <div class="col-md-8 offset-2">
  <h2>login page</h2>
  <p>Chania is the capital of the Chania region on the island of Crete. The city can be divided in two parts, the old town and the modern city.</p></div>
  
  <div class="col-md-8 offset-2">
  <p>Resize the browser window to see how the content respond to the resizing.</p>
  </div>
  <div class="col-md-8 offset-2">
  <p>Resize the browser window to see how the content respond to the resizing.</p>
  </div>
  <div class="col-md-8 offset-2">
  <p>Resize the browser window to see how the content respond to the resizing.</p>
  </div>

  <div class="col-md-8 offset-2">
  <p>Chania is the capital of the Chania region on the island of Crete. The city can be divided in two parts, the old town and the modern city.</p></div>
  
  <div class="col-md-8 offset-2">
  <p>Resize the browser window to see how the content respond to the resizing.</p>
  </div>
  <div class="col-md-8 offset-2">
  <p>Resize the browser window to see how the content respond to the resizing.</p>
  </div>
  <div class="col-md-8 offset-2">
  <p>Resize the browser window to see how the content respond to the resizing.</p>
  </div>
  
<!-- client master div -->
  <div class="row" id="clientmaster">
  <div class="col-md-8 offset-2">
  <h2>login page</h2>
  <p>Chania is the capital of the Chania region on the island of Crete. The city can be divided in two parts, the old town and the modern city.</p></div>
  
  <div class="col-md-8 offset-2">
  <p>Resize the browser window to see how the content respond to the resizing.</p>
  </div>
  <div class="col-md-8 offset-2">
  <p>Resize the browser window to see how the content respond to the resizing.</p>
  </div>
  <div class="col-md-8 offset-2">
  <p>Resize the browser window to see how the content respond to the resizing.</p>
  </div>

  <div class="col-md-8 offset-2">
  <p>Chania is the capital of the Chania region on the island of Crete. The city can be divided in two parts, the old town and the modern city.</p></div>
  
  <div class="col-md-8 offset-2">
  <p>Resize the browser window to see how the content respond to the resizing.</p>
  </div>
  <div class="col-md-8 offset-2">
  <p>Resize the browser window to see how the content respond to the resizing.</p>
  </div>
  <div class="col-md-8 offset-2">
  <p>Resize the browser window to see how the content respond to the resizing.</p>
  </div>

   <!-- complain Details Div -->


  <div class="row" id="complaindetails">
  <div class="col-md-8 offset-2">
  <p>Chania is the capital of the Chania region on the island of Crete. The city can be divided in two parts, the old town and the modern city.</p></div>
  
  <div class="col-md-8 offset-2">
  <p>Resize the browser window to see how the content respond to the resizing.</p>
  </div>
  <div class="col-md-8 offset-2">
  <p>Resize the browser window to see how the content respond to the resizing.</p>
  </div>
  <div class="col-md-8 offset-2">
  <p>Resize the browser window to see how the content respond to the resizing.</p>
  </div>

  <div class="col-md-8 offset-2">
  <p>Chania is the capital of the Chania region on the island of Crete. The city can be divided in two parts, the old town and the modern city.</p></div>
  
  <div class="col-md-8 offset-2">
  <p>Resize the browser window to see how the content respond to the resizing.</p>
  </div>
  <div class="col-md-8 offset-2">
  <p>Resize the browser window to see how the content respond to the resizing.</p>
  </div>
  <div class="col-md-8 offset-2">
  <p>Resize the browser window to see how the content respond to the resizing.</p>
  </div>

  <!-- client report div start-->

  <div class="row" id="clientreport">
  <div class="col-md-8 offset-2">
  <p>Chania is the capital of the Chania region on the island of Crete. The city can be divided in two parts, the old town and the modern city.</p></div>
  
  <div class="col-md-8 offset-2">
  <p>Resize the browser window to see how the content respond to the resizing.</p>
  </div>
  <div class="col-md-8 offset-2">
  <p>Resize the browser window to see how the content respond to the resizing.</p>
  </div>
  <div class="col-md-8 offset-2">
  <p>Resize the browser window to see how the content respond to the resizing.</p>
  </div>

  <div class="col-md-8 offset-2">
  <p>Chania is the capital of the Chania region on the island of Crete. The city can be divided in two parts, the old town and the modern city.</p></div>
  
  <div class="col-md-8 offset-2">
  <p>Resize the browser window to see how the content respond to the resizing.</p>
  </div>
  <div class="col-md-8 offset-2">
  <p>Resize the browser window to see how the content respond to the resizing.</p>
  </div>
  <div class="col-md-8 offset-2">
  <p>Resize the browser window to see how the content respond to the resizing.</p>
  </div>

</div>
</body>
</html>